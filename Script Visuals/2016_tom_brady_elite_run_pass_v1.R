...
df <- subset(df, timeout.flag =  0)
df <- subset(df, intercepted.flag  =  0)
df <- subset(df, fumble.flag  =  0)
df <- subset(df, challenge.flag  =  0)
df <- subset(df, kick_flag  =  0)
df <- subset(df, punt_flag  =  0)
...
first_downs <- subset(first_downs, Score.Diff >= -10 & Score.Diff <= 10)
by_ydline <- summarize(group_by(first_downs, ABS.Yardline), raw_ep = mean(nextscore), count = n())
#view results of avg by yard line
plot(by_ydline$ABS.Yardline, by_ydline$raw_ep)
plot(by_ydline$ABS.Yardline, by_ydline$count)
...
#view result
plot(by_ydline$ABS.Yardline, by_ydline$smooth_ep)
